...
class SpeechController {
  async handleWebSocket(connection: any, request: FastifyRequest) {
    ...
    connection.socket.on('message', async (message: Buffer | string) => {
      ...
      request.log.debug('Received message from client:', {
        isBuffer: Buffer.isBuffer(message),
        isString: typeof message === 'string',
        type: typeof message,
        length: Buffer.isBuffer(message) ? message.length : typeof message === 'string' ? message.length : undefined
      ...
      if (isJSON && parsedData) {
        ...
        if (data.type === 'Start' && !started) {
          ...
          try {
            request.log.info('Creating Deepgram connection with options:', {
            ...
            request.log.info('Deepgram connection object created:', {
            ...
            request.log.info('Deepgram events object:', {
              Open: events.Open,
            ...
            request.log.info('Setting up Open event handler with event name:', openEventName)
            ...
            request.log.info('Setting up Transcript event handler with event name:', transcriptEventName)

            dgConnection.on(transcriptEventName, (evt: any) => {
              request.log.info('Deepgram transcript event received:', {
            ...
            request.log.info('All Deepgram event handlers set up. Connection state:', {
              hasOn: typeof (dgConnection as any).on === 'function',
              connectionType: typeof dgConnection,
